...
Local development infrastructure for PokerMap — the root `docker-compose.yml` and supporting tooling that gives every contributor a one-command path to a running stack (Postgres, Redis, MinIO, and any app services) with credentials, ports, and database names that stay wire-compatible with the production VPS platform defined in [[shared-data-services]] and [[vps-platform]].
## Requirements
